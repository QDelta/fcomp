...
byte_t *now_heap;
int64_t now_heap_cap;
int64_t now_heap_size;

byte_t *to_heap;
int64_t to_heap_cap;
int64_t to_heap_size;

int64_t stat_gc_count = 0;
int64_t stat_alloc_count = 0;

void global_gc(int64_t reserve_size);
...
void heap_init(void) {
    now_heap = malloc(HEAP_INIT_CAP);
    now_heap_cap = HEAP_INIT_CAP;
    now_heap_size = 0;
    to_heap = NULL;
    to_heap_cap = 0;
    to_heap_size = 0;
...
node_ptr node_alloc(int64_t d_num) {
    ...
    if (now_heap_size + alloc_size > now_heap_cap) {
        global_gc(alloc_size);
    }

    node_ptr new_node = (node_ptr)(now_heap + now_heap_size);
    now_heap_size += alloc_size;
...
node_ptr gc_copy(node_ptr n) {
    if (! n || n->gc_isglobal)
        ...
        node_ptr forwarding = (node_ptr)(to_heap + to_heap_size);
...
void global_gc(int64_t reserve_size) {
    to_heap_cap = max(now_heap_cap * 1.5, now_heap_cap + reserve_size);
    to_heap_size = 0;
    to_heap = malloc(to_heap_cap);
    ...
    int64_t expect_heap_cap = to_heap_size + max(to_heap_size, reserve_size);
    if (to_heap_cap >= expect_heap_cap) {
        to_heap = realloc(to_heap, expect_heap_cap);
        to_heap_cap = expect_heap_cap;
    }

    free(now_heap);
    now_heap = to_heap;
    now_heap_size = to_heap_size;
    now_heap_cap = to_heap_cap;
...
void print_stat(void) {
    printf("Heap: Node allocation: %ld\n", stat_alloc_count);
    printf("Heap: Now size: %ld\n", now_heap_size);
    printf("Heap: Now capacity: %ld\n", now_heap_cap);
    printf("GC: Count: %ld\n", stat_gc_count);
}
